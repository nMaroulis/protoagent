...
| --- | --- | --- | --- | --- |
| Rust CLI / TUI | `proto-cli` | `0.2.3` | Active | `cli/Cargo.toml` |
| Python core | `protoagent-core` | `0.2.3` | Active | `core/pyproject.toml` and `protoagent_core.__version__` |
| ACP server | `proto-acp` | `0.0.0-dev.0` | Planned | `acp/VERSION` |
| Documentation | `protoagent-docs` | `0.2.3` | Active release metadata | `docs/package.json` |
...
“ProtoAgent 0.2.3” is the coordinated release train for the active CLI and
...
ProtoAgent 0.2.3 requires `protolink>=0.7.1` for native delegated receipts, persistence redaction, checkpoint inventory and live streaming.
Publish ProtoLink 0.7.1 to the target package index first, verify that a clean
environment can resolve it, and only then publish ProtoAgent core/CLI 0.2.3
